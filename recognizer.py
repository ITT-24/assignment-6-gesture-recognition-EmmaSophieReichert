...
class Result:
    ...
    def __repr__(self):
        return f"Result(Name='{self.name}', Score={self.Score}, Time={self.Time} ms)"
...
def centroid(points) -> Point: #finished
    x = 0.0
    y = 0.0
...
def indicative_angle(points): #finished
    c = centroid(points)
    return math.atan2(c.y - points[0].y, c.x - points[0].x)

def rotate_by(points, radians): #finished
    c = centroid(points)
...
def scale_to(points, square_size): #finished
